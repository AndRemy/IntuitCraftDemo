...
tweets_draft <- read.csv("Bookkeeping_Tweets_2021-06-04.csv", stringsAsFactors = FALSE)
...
# Token
token_bk <- tweets["text"] %>%
  unnest_tokens(word, text) %>%
  anti_join(custom_stop_words) %>%
  count(word, sort=TRUE) %>%
  inner_join(afinn, by=c(word="word"))

token_bk$value <- token_bk$value * token_bk$n

wordcloud(words = token_bk$word,
          freq = token_bk$n,
          min.freq = 1,
          max.words=200,
          random.order=FALSE,
...
ggraph(trigram_counts, layout = "fr") +
  geom_edge_link()+
  geom_node_point()+
  geom_node_text(aes(label=name), vjust =1, hjust=1)
